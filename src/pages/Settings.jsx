    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center py-8 px-4 font-sans">
      <div className="max-w-5xl w-full">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex-grow flex flex-col">
              ...
              <div className="mb-4 p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex gap-2 mb-2">
                  <input type="text" value={saveName} onChange={(e) => setSaveName(e.target.value)} placeholder="保存名 (例: 1年A組)" className="flex-grow p-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:border-blue-400 bg-white" />
                  <button onClick={handleSaveList} className="px-4 py-2 bg-blue-100 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-200 active:scale-95 transition-all whitespace-nowrap">保存する</button>
                </div>
                {savedLists.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-slate-200">
                    {savedLists.map(list => (
                      <div key={list.id} className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                        <button onClick={() => handleLoadList(list)} className="px-3 py-1.5 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">{list.name}</button>
                        <button onClick={() => handleDeleteList(list.id)} className="px-2 py-1.5 bg-slate-50 text-slate-300 hover:bg-red-100 hover:text-red-500 transition-colors border-l border-slate-100">✕</button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <textarea
                ...
                value={namesText} onChange={handleNamesChange} placeholder="山田太郎&#13;&#10;佐藤花子" style={{ minHeight: '120px' }}
              />
            </div>
          </div>
          ...
          <div className="space-y-6 flex flex-col relative">
            ...
            <div className="flex items-center justify-between p-5 bg-white rounded-2xl shadow-sm border border-slate-100">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" checked={showProgressToAll} onChange={() => setShowProgressToAll(!showProgressToAll)} />
        ...
        <div className="flex justify-center max-w-md mx-auto">
          <button onClick={handleStart} className="w-full py-4 rounded-2xl bg-blue-500 text-white font-bold text-lg shadow-[0_4px_0_rgb(59,130,246,0.5)] hover:bg-blue-400 hover:shadow-[0_4px_0_rgb(96,165,250,0.5)] active:translate-y-1 active:shadow-none transition-all">
            授業をスタートする
          </button>
        </div>
      </div>
